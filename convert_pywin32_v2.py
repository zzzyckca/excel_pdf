...
# CONFIGURATION
# ==========================================
# Define your input folder here. PDFs will be saved in the same directory (on the network).
INPUT_DIR = r"c:\Users\yckde\Documents\GitHub\excel_pdf\test"
# Define the local temporary directory for processing
TEMP_DIR = r"c:\temp5"
...
def convert_file(file_path):
    """
    Worker function to convert a single Excel file to PDF locally.
...
def main():
    ...
    try:
        ...
        # --- NEW IN V2: LOCAL COPY STEP ---
        print(f"\n[*] Preparing local temporary workspace at {TEMP_DIR}...")
        if local_temp_dir.exists():
            shutil.rmtree(local_temp_dir, ignore_errors=True)
        local_temp_dir.mkdir(parents=True, exist_ok=True)
        
        print(f"[*] Copying {len(excel_files)} file(s) from network to local drive to avoid throughput issues...")
        local_excel_files = []
        for mapped_file_str in excel_files:
            mapped_file = Path(mapped_file_str)
            rel_path = mapped_file.relative_to(mapped_path_obj)
            local_target = local_temp_dir / rel_path
            
            # Ensure local subdirectories exist
            local_target.parent.mkdir(parents=True, exist_ok=True)
            
            # Copy the file
            shutil.copy2(mapped_file, local_target)
            local_excel_files.append(str(local_target))
            
        print("[*] Copy complete. Beginning local processing.\n")
        # -----------------------------------
        ...
        # Run the multiprocessing pool using the LOCAL files
        with multiprocessing.Pool(processes=num_processes) as pool:
            # pool.imap_unordered yields results as soon as they are ready
            for result in pool.imap_unordered(convert_file, local_excel_files):
                success, filename, local_abs_path, err_msg, start_dt, end_dt, elapsed = result
                
                local_excel_path = Path(local_abs_path)
                local_pdf_path = local_excel_path.with_suffix('.pdf')
                
                # Determine remote paths for copy-back and reporting
                rel_path = local_excel_path.relative_to(local_temp_dir)
                remote_excel_path = mapped_path_obj / rel_path
                remote_pdf_path = remote_excel_path.with_suffix('.pdf')
                
                # --- NEW IN V2: COPY PDF BACK TO NETWORK ---
                if success and local_pdf_path.exists():
                    try:
                        shutil.copy2(local_pdf_path, remote_pdf_path)
                    except Exception as e:
                        success = False
                        err_msg = f"Conversion succeeded locally, but failed to copy PDF back to network: {e}"
                # -------------------------------------------
                
                # Translate the mapped drive back to the original path for the report
                original_full_path = str(remote_excel_path).replace(MAPPED_DRIVE_LETTER, INPUT_DIR, 1)
        ...
        print(f"\n[*] Cleaning up local temporary workspace '{TEMP_DIR}'...")
        try:
            if local_temp_dir.exists():
                shutil.rmtree(local_temp_dir, ignore_errors=True)
